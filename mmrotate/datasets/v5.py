...
class V5Dataset(CustomDataset):
    """AIHUB dataset for detection.
    ...
    """
    # CLASSES = ('etc', 'ship(S)', 'ship(L)', 'aircraft(C)', 'aircraft(M)',
    #            'small car', 'bus', 'truck', 'train', 'crane', 'bridge',
    #            'storage tank', 'dam', 'stadium', 'sports field', 'helipad',
    #            'rotary', 'helicopter', 'container(I)', 'container(G)',
    #            'swimming pool')
    
    CLASSES = ("etc", "ship", "aircraft", "vehicle", "crane",
                "bridge", "storage-tank", "dam", "stadium", "sports-field",
                "helipad", "rotary", "helicopter", "container", "swimming-pool")
    ...
    def __init__(self,
        ...
        super(V5Dataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        ...
        """
        # print(f"[DEBUG] [load_annotations] 진입 - ann_folder: {ann_folder}")

        cls_map = {c: i
                   for i, c in enumerate(self.CLASSES)
                   }  # in mmdet v2.0 label is 0-based
        ann_files = glob.glob(ann_folder + '/*.txt')
        # print(f"[DEBUG] Found {len(ann_files)} annotation files in {ann_folder}")
        
        data_infos = []
        if not ann_files:  # test phase
            print("[WARNING] No annotation files found! Check ann_file path.")
            ...
            for ann_file in ann_files:
                # print(f"[DEBUG] 예시 파일: {ann_file} 크기 {os.path.getsize(ann_file)} bytes")
                data_info = {}
                img_id = osp.split(ann_file)[1][:-4]
                img_name = img_id + '.png'
                data_info['filename'] = img_name
                ...
                with open(ann_file) as f:
                    s = f.readlines()
                    # print(f"[DEBUG] Reading {ann_file} with {len(s)} lines")
                    for si in s:
                        # bbox_info = re.split(r'[,\s]+', si.strip())
                        # si = re.sub(r',\s+', ',', si.strip())
                        # bbox_info = si.split(',')
        ...
        # print(f"[DEBUG] Loaded {len(data_infos)} annotations from {ann_folder}")
        return data_infos
